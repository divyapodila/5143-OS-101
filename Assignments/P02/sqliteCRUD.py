...
class SQLiteCrud:
    ...
    # Description for `search` method:
    # query a table based on specified conditions and retrieve data that matches those conditions.
    # 1. `table_name (str)`: The name of the table to search within.
    # 2. `conditions (dict)`: A dictionary where keys represent column names,
    #    and values are the specific values you want to match in those columns.
    # 3. `raw (bool)`: An optional parameter indicating whether the results in raw format
    #    (as a list of tuples) or a formatted PrettyTable.
    # The method first constructs a WHERE clause using the provided conditions,
    # and then it executes an SQL SELECT query to retrieve the data.
    #  If `raw` is set to `False`, it formats the results using the `__formatted_results` method
    #  to return a structured PrettyTable. If an SQLite error occurs during the search,
    #  it handles the error and prints an error message, ensuring robustness in querying the database

    def search(self, table_name, conditions, raw=True):
        """Search for data in a table based on conditions.

        Args:
            table_name (str): Name of the table.
            conditions (dict): Dictionary of column-value pairs for conditions.
            raw (bool): Whether to return raw results or formatted table.
        """
        try:
